...
def initial0():
    ...
    bdy0 = [1,3]   # Dirichlet data at x=0 and y = 0
    bdy1 = [2,4]   # Neumann   data at x=1 and y = 1
...
##################################################
# ****************Symmetric flow and antisymmetric director with eta<0************************

# **************** Example 1 ************************
eg1 = Example("Guess Flow For perturbation on direction with eta<0 ")
...
def initial1():
    ...
    bdy0 = [1,2]   # Dirichlet data at x=0 and y = 0
    bdy1 = [3,4]   # Neumann   data at x=1 and y = 1
...
def exact1(t,xx):
    ...
    v = np.array( [z,0.001*(1-xx[0]*xx[0])])
    dvt = np.array( [z,z])
    dvx = np.array([[z,2*0.001*(-xx[0])], [z,z]])
...
def exact3(t,xx):
    ...
    v = np.array( [z, -theta])
    dvt = np.array( [z,z])
    dvx = np.array([[z,-2*c0*pi*cos(pi*xx[0])], [z,z]])
    ...
    dpt = z;
    dpx = np.array([z,z])
    omega = c0 * sin(3*pi*xx[0])
    d   = np.array([sin(-omega), cos(-omega)])
    ddt = np.array( [z,z])
    ddx = np.array([[z,z], [z,z]])
    return ([v,p,d], [dvx,dpx,ddx], [dvt,dpt,ddt])
...
# **************** Example 4 ************************
# ****************Symmetric director and antisymmetric flow
eg4 = Example("Flow For perturbation on direction with eta<0 ")

def initial4():
    eg4.PureDirichletBc = False;
    eg4.eta = -1;
...
eg4.initialize = initial4; eg4.exact = exact4; eg4.rhs = rhs4
#####################################
Examples = [eg0, eg1, eg2, eg3, eg4]
